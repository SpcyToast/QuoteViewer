...
class QuoteSelectorTest {
    ...
    fun `fetchQuote - get quote from API call - success`()= runTest{
        whenever(quoteClient.getQuote()).thenReturn(Quote("Test Quote","Test Author","Test Category"))
        assertEquals(quoteSelector.fetchQuote(),Result.success(Quote("Test Quote","Test Author","Test Category")))
    }
}